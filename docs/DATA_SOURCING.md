...
matters until a dataset passes `scripts/run_data_preflight.py` (see
...
| 3 | **Historical constituent membership** | Every entry AND exit date per symbol per period, including symbols that were later removed. Build via `scripts/build_membership_manifest.py` from a vendor CSV (`symbol,start,end`) | CRITICAL FAIL without any manifest; FAIL on zero exits / static membership |
...
# 3) only now, per family (same manifest removes the survivorship ceiling legitimately)
uv run python scripts/run_strategy_research.py \
    --family trend_relative_strength --data-dir <daily-bars> --benchmark SPY \
    --symbols <manifest symbols...> --output-dir artifacts/research
```
